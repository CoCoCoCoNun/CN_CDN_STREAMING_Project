...
class HQServer:
    ...
    def init(self):
        ...
        self.sock.bind(config['HQ_server'])
    ...
    def make_random_delay(self):
        r = random.random()
        if self.chunk_idx == 10:
            return time.time() + random.uniform(15,20)  #데모 시나리오를 위한 장치
        if r < 0.05:                          
    ...
    def run(self):   
        while True:
            ...
            if self.streaming and self.chunk_idx >= len(self.chunks):
                self.streaming = False
                print('스트리밍 완료')
