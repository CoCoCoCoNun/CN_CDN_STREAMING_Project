...
class ClientFSM:
    ...
    def handle_wait_video_index(self):
        ...
        print(f'동영상 URL: {self.video_index} from {addr}')
        input()
    ...
    def handle_wait_manifest(self):
        ...
        self.manifest = data
        print(f'manifest 파일: {data} from {addr}')
        input()
   ...
   #STREAMING: 스트리밍 시작(최초 재생 스레드 시작) 및 probe와 화질 전환 검사
    ...
    def _play_loop(self):
        ...
        while self.state not in (State.END_STREAMING, State.ERROR):
            ...
            print(f'재생: quality={chunk["quality"]} start_time={chunk["start_time"]} end_time={chunk["end_time"]}')  # 추가
    ...
    def handle_wait_quality_change(self):
        ...
        print(f'k={K}, β={QUALITY_DOWN}, γ={QUALITY_UP}')
        print(f'ROF={self.buffer_rate:.3f}')
